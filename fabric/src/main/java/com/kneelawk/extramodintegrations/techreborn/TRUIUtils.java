...
public class TRUIUtils {
    public static void energyBar(WidgetHolder widgets, RebornRecipe recipe, int machineEnergy, int x, int y) {
        widgets.addTexture(TRTextures.ENERGY_BAR_EMPTY, x, y).tooltip((mx, my) -> List.of(
            ClientTooltipComponent.create(tooltip("techreborn.recipe_power", recipe.power()).getVisualOrderText())));
        widgets.addAnimatedTexture(TRTextures.ENERGY_BAR_FULL, x, y, machineEnergy * 1000 / recipe.power() * 50, false, true, true);
    }
